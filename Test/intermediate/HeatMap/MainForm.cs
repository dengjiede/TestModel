...
namespace HeatMap
{
    public partial class MainForm : Form
    {
        ...
        List<double> keyDatetimesList = new List<double>();
        ...
        public MainForm()
        {
            ...
            try
            {
                IConnectionInfo ci = new ConnectionInfo();
                ci.ConnectionType = gviConnectionType.gviConnectionFireBird2x;
                string tmpFDBPath = Path.Combine(Application.StartupPath.Substring(0, flag), @"Samples\Media\HeatMap.FDB");
                ci.Database = tmpFDBPath;
                IDataSourceFactory dsFactory = new DataSourceFactory();
                IDataSource ds = dsFactory.OpenDataSource(ci);
                string[] setnames = (string[])ds.GetFeatureDatasetNames();
            ...
            else
            {
                ...
                {
                    DateTime d = DateTime.FromOADate(keyDatetimesList[t]);
            ...
            if (keyDatetimesList.Count > 0)
            {
                DateTime dmax = DateTime.FromOADate(keyDatetimesList[keyDatetimesList.Count - 1]);
                player.SetTime(dmax);
        ...
        private void btnPlay_Click(object sender, EventArgs e)
        {
            if (this.btnPlay.Text == "播放")
            {
                ...
                DateTime startTime = DateTime.FromOADate(keyDatetimesList[0]);
                DateTime endTime = DateTime.FromOADate(keyDatetimesList[keyDatetimesList.Count - 2]);
        ...
        {
            DateTime d = DateTime.FromOADate(keyDatetimesList[this.cbKeyTimes.SelectedIndex]);
            player.SetTime(d);
        }

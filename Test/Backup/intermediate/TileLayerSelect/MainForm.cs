        ...
        void axRenderControl1_RcMouseClickSelect(object sender, Gvitech.CityMaker.Controls._IRenderControlEvents_RcMouseClickSelectEvent e)
        {
            ...
            if (e.eventSender == gviMouseSelectMode.gviMouseSelectClick)
            {
                ...
                if (pr.Type == gviObjectType.gviObject3DTileLayer)
                {
                    if(Mode == 1)
                    {
                        IPoint intersectPoint = e.intersectPoint;
                        ...
                        foreach (IFeatureClass fc in fcMap.Keys)
                        {
                            ...
                            if (e.mask != gviModKeyMask.gviModKeyCtrl && e.mask != gviModKeyMask.gviModKeyShift)
                                if (list.Count > 0)
                                    break;
                        }
                    }
                    else if(Mode == 2)
                    {
                        ...
                        double length = disOperator.Distance3D(e.intersectPoint);
                        //向相机方向延伸n米:n跟眼睛到交点距离有关，当距离远时n大，当距离近时n小。
                        factor = length * 0.001;

                        IPoint aimingPoint = this.axRenderControl1.Camera.GetAimingPoint2(e.intersectPoint, cameraAngle, factor);
                        IPoint sourcePoint = this.axRenderControl1.Camera.GetAimingPoint2(e.intersectPoint, cameraAngle, -factor);
                        ...
                        //intersetPolyline.AppendPoint(e.intersectPoint);
                        ...
                        //intersetPolyline2.AppendPoint(e.intersectPoint);
                        //cs.Color = 0xffff0000;

    ...
    public partial class MainForm : Form
    {
        ...
        public MainForm()
        {
            ...
            try
            {
                ...
                string tmpFDBPath = Path.Combine(Application.StartupPath.Substring(0, flag), @"Samples\Media\Network.FDB");
            ...
            try
            {
                IConnectionInfo ci = new ConnectionInfo();
                ci.ConnectionType = gviConnectionType.gviConnectionFireBird2x;
                string tmpFDBPath = Path.Combine(Application.StartupPath.Substring(0, flag), @"Samples\Media\POI-1.FDB");
                ci.Database = tmpFDBPath;
                IDataSourceFactory dsFactory = new DataSourceFactory();
                IDataSource ds = dsFactory.OpenDataSource(ci);
                string[] setnames = (string[])ds.GetFeatureDatasetNames();
            ...
            string[] fn = Directory.GetFiles(Path.Combine(Application.StartupPath.Substring(0, flag), @"Samples\Media\xml\xml"));
            ...
            textAttribute.TextColor = 0xaa0000ff;
        ...
        private void LoadCar(IDynamicObject dynamicObject)
        {
            if (renderModelPoint == null)
            {
                ...
                string modelName = Path.Combine(Application.StartupPath.Substring(0, flag), @"Samples\Media\osg\Vehicles\Car\TCJ006.osg");
        ...
        private void LoadPeople(IDynamicObject dynamicObject)
        {
            if (skinMesh == null)
            {
                // 构造ModelPoint
                mp = geoFac.CreateGeometry(gviGeometryType.gviGeometryModelPoint, gviVertexAttribute.gviVertexAttributeZ) as IModelPoint;
                string modelName = Path.Combine(Application.StartupPath.Substring(0, flag), @"Samples\Media\x\Character\QiYeYuanGong.X");
                mp.ModelName = modelName;
                mp.SpatialCRS = dataset_Road.SpatialReference;
                // 设置位置
                IMatrix matrix = new MatrixClass();
                        ...
                        if (route != null)
                        {
                            ...
                            for (int j = 0; j < segmentCount; j++)
                            {
                                ...
                                foreach (IFeatureClass fc in fcMap_POI.Keys)
                                {
                                    ...
                                    {
                                        this.axRenderControl1.FeatureManager.HighlightFeature(fc, int.Parse(strs[1]), 0xffff0000);
                                        break;
                                    }
                                }

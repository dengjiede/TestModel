    ...
    public partial class MainForm : Form
    {
        ...
        public MainForm()
        {
            ...
            try
            {
                ...
                string tmpFDBPath = Path.Combine(Application.StartupPath.Substring(0, flag), @"Samples\Media\empty.FDB");
        ...
        private void menuGdbImport_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                string sGdbFilePath = "";
                if (flag > -1)
                    sGdbFilePath = Path.Combine(Application.StartupPath.Substring(0, flag), @"Samples\Media\gdb\gdbtest.gdb");
                else
                {
                    MessageBox.Show("请不要随意更改SDK目录名");
                    return;
                ...
                if (ext.ToLower().Equals("gdb"))
                {
                    ...
                    if (op != null)
                    {
                        if (op.LayersInfo.Count != 0)
                        {
                            ...
                            ISpatialCRS coorSysProject = coorFactory.CreateFromWKT(op.LayersInfo[0].CrsWKT) as ISpatialCRS;
                            if (fds.SpatialReference.Name.Equals("\\unnamed\\") || fds.SpatialReference.IsSame(coorSysProject))
                            {
                                ...
                                if (b)
                                {
                                    ...
                                    fcuidMap.Add(newFc.GuidString, newFc);
            ...
            try
            {
                ...
                if (dlg.ShowDialog() == DialogResult.OK)
                {
                    ...
                    else if (ext.ToLower().Equals("dwg"))
                    {
                        ...
                        {
                            string s = string.Format("{0}-{1}", prefix.Replace('.', '-'), op1.LayersInfo[i].Name);
                            //判断要名称为fn的FeatureClass是否已存在，如果存在，fn名称添加后缀
                            if (collection != null)
                            {
                            ...
                            list.Add(op1.LayersInfo[i].Name, s);
                        }
        ...
        private void ImportSHP(object param)
        {
            ...
            try
            {
                ...
                op = dintf.CreateDataInterop(
                            gviDataConnectionType.gviOgrConnectionShp,
                             ps);
                IFieldInfoCollection fields = op.LayersInfo[0].FieldInfos;
                ...
                if (ds != null)
                {
                    ...
                    if (fds != null)
                    {
                        ...
                        strMap.SetProperty(op.LayersInfo[0].Name, fn);
            ...
            try
            {
                ...
                if (ds != null)
                {
                    ...
                    if (fds != null)
                    {
                        ...
                        op.StepValue = 1;
                        IPropertySet strMap = new PropertySet();
                        strMap.SetProperty(op.LayersInfo[0].Name, dstFcn);
                        int count = op.ImportLayers(fds, strMap);
                        if (count == 1)
                            bar.Invoke(_IsFinishDelegate, true);
                        else

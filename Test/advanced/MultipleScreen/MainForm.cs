...
namespace MultipleScreen
{
    public partial class MainForm : Form
    {
        ...
        public MainForm()
        {
            ...
            g.Viewport.ViewportMode = gviViewportMode.gviViewportQuad;

            this.cbCompassVisibleMask.SelectedIndex = 5;
            this.cbCameraBind.Checked = true;
            this.cbShowBorder.Checked = true;
            g.Viewport.CameraViewBindMask = gviViewportMask.gviViewAllNormalView;
        ...
        private void LoadData()
        {
            ...
            string modelName = Path.Combine(Application.StartupPath.Substring(0, flag), @"Samples\Media\osg\Schemes\scheme-01.osg");
            ...
            renderModelPoint1 = g.ObjectManager.CreateRenderModelPoint(modelPoint, null, rootId);
            ...
            IEulerAngle angle = new EulerAngle();
            angle.Set(0, -20, 0);
            g.Camera.LookAt2(point, 300, angle);

            modelName = Path.Combine(Application.StartupPath.Substring(0, flag), @"Samples\Media\osg\Schemes\scheme-02.osg");
            ...
            modelPoint.SetCoords(dx, dy, dz - 15, 0, 0);
            modelPoint.SpatialCRS = _datasetCRS;
            renderModelPoint2 = g.ObjectManager.CreateRenderModelPoint(modelPoint, null, rootId);
            renderModelPoint2.VisibleMask = gviViewportMask.gviView1;


            modelName = Path.Combine(Application.StartupPath.Substring(0, flag), @"Samples\Media\osg\Schemes\scheme-03.osg");
            modelPoint = (IModelPoint)geoFactory.CreateGeometry(gviGeometryType.gviGeometryModelPoint, gviVertexAttribute.gviVertexAttributeZ);
            modelPoint.ModelName = modelName;
            modelPoint.SetCoords(dx, dy, dz - 10, 0, 0);
            modelPoint.SpatialCRS = _datasetCRS;
            renderModelPoint3 = g.ObjectManager.CreateRenderModelPoint(modelPoint, null, rootId);
            renderModelPoint3.VisibleMask = gviViewportMask.gviView2;


            modelName = Path.Combine(Application.StartupPath.Substring(0, flag), @"Samples\Media\osg\Schemes\scheme-04.osg");
            modelPoint = (IModelPoint)geoFactory.CreateGeometry(gviGeometryType.gviGeometryModelPoint, gviVertexAttribute.gviVertexAttributeZ);
            modelPoint.ModelName = modelName;
            modelPoint.SetCoords(dx, dy, dz - 13, 0, 0);
            modelPoint.SpatialCRS = _datasetCRS;
            renderModelPoint4 = g.ObjectManager.CreateRenderModelPoint(modelPoint, null, rootId);
        ...
        private void radioButtonMode_CheckedChanged(object sender, EventArgs e)
        {
            ...
            g.Viewport.ActiveView = 0;
            renderModelPoint1.VisibleMask = gviViewportMask.gviView0;

            if (radioButtonQuad.Checked)
                g.Viewport.ViewportMode = gviViewportMode.gviViewportQuad;
            else if (radioButtonQuadH.Checked)
                g.Viewport.ViewportMode = gviViewportMode.gviViewportQuadH;
            else if (radioButtonT1B1.Checked)
                g.Viewport.ViewportMode = gviViewportMode.gviViewportT1B1;
            else if (radioButtonT1M1B1.Checked)
                g.Viewport.ViewportMode = gviViewportMode.gviViewportT1M1B1;
            else if (radioButtonL1R1.Checked)
                g.Viewport.ViewportMode = gviViewportMode.gviViewportL1R1;
            else if (radioButtonL1M1R1.Checked)
                g.Viewport.ViewportMode = gviViewportMode.gviViewportL1M1R1;
            else if (radioButtonL1R2.Checked)
                g.Viewport.ViewportMode = gviViewportMode.gviViewportL1R2;
            else if (radioButtonL2R1.Checked)
                g.Viewport.ViewportMode = gviViewportMode.gviViewportL2R1;
            else if (radioButtonL1R1SingleFrustum.Checked)
                g.Viewport.ViewportMode = gviViewportMode.gviViewportL1R1SingleFrustum;
            else if (radioButtonT1B1SingleFrustum.Checked)
                g.Viewport.ViewportMode = gviViewportMode.gviViewportT1B1SingleFrustum;
            else if (radioButtonPIP.Checked)
            {
                //获取原始相机位置                
                IVector3 cameraPos = new Vector3();
                IEulerAngle cameraAngle = new EulerAngle();
                g.Camera.GetCamera(out cameraPos, out cameraAngle);

                //设值PIP视口相机位置
                g.Viewport.ViewportMode = gviViewportMode.gviViewportPIP;
                g.Viewport.ActiveView = 4;
                renderModelPoint1.VisibleMask = gviViewportMask.gviViewPIP;
                //注意：PIP窗口不支持FlyToObject，支持SetCamera和LookAt
                //g.Camera.FlyToObject(renderModelPoint1.Guid, gviActionCode.gviActionFlyTo);
                g.Camera.SetCamera(cameraPos, cameraAngle, gviSetCameraFlags.gviSetCameraNoFlags);
                //g.Camera.LookAt(cameraPos, 50, cameraAngle);
            }
            ...
            {
                g.Viewport.CameraViewBindMask = gviViewportMask.gviViewNone;
            }
        }
        ...
        private void cbCameraInfoVisible_CheckedChanged(object sender, EventArgs e)
        {
            ...
            {
                g.Viewport.CameraInfoVisible = true;
            }
            else
            {
                g.Viewport.CameraInfoVisible = false;
            }
        }
        ...
        private void cbCompassVisibleMask_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                case 0:
                    g.Viewport.CompassVisibleMask = gviViewportMask.gviViewNone;
                    break; 
                case 1:
                    g.Viewport.CompassVisibleMask = gviViewportMask.gviView0;
                    break;
                case 2:
                    g.Viewport.CompassVisibleMask = gviViewportMask.gviView1;
                    break;
                case 3:
                    g.Viewport.CompassVisibleMask = gviViewportMask.gviView2;
                    break;
                case 4:
                    g.Viewport.CompassVisibleMask = gviViewportMask.gviView3;
                    break;
                case 5:
                    g.Viewport.CompassVisibleMask = gviViewportMask.gviViewAllNormalView;
                    break;
                case 6:
                    g.Viewport.CompassVisibleMask = gviViewportMask.gviViewPIP;
                    break;
            }
        }
        ...
        private void MainForm_Load(object sender, EventArgs e)
        {
            ...
            splitTrackH.Value = (int)(g.Viewport.SplitRatioH * 100);
            ...
            splitTrackV.Value = (int)(g.Viewport.SplitRatioH * 100);

...
namespace ImageryLayer
{
    ...
    public partial class MainForm : Form
    {
        ...
        public MainForm()
        {
            ...
            // 注册地形
            string tmpTedPath = Path.Combine(Application.StartupPath.Substring(0, flag), @"Samples\Media\terrain\world.ted");
            ...
            // 创建影像图层
            string tmpImageryPath = Path.Combine(Application.StartupPath.Substring(0, flag), @"Samples\Media\imagery\jk1.tif");
            ...
            IRasterSource source = rasFac.OpenRasterSource(@"http://192.168.2.176:6080/arcgis/services/xinjiapo/MapServer/WMSServer", gviRasterConnectionType.gviRasterConnectionWMS);
            if (source != null)
            {
                string[] names = source.GetRasterNames();
                IRaster raster = source.OpenRaster(names[0]);
                IImageryLayer imagelayer2 = this.axRenderControl1.ObjectManager.CreateImageryLayer(raster.ConnStr, rootId);
                this.axRenderControl1.Camera.FlyToObject(imagelayer2.Guid, gviActionCode.gviActionFlyTo);
                // 添加节点到界面控件上
        ...
        private void toolStripButtonAddImageryLayer_Click(object sender, System.EventArgs e)
        {
            try
            {
                ...
                {
                    gviRasterSourceType selectType;

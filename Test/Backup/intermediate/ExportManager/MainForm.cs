...
namespace ExportManager
{
    public partial class MainForm : Form
    {
        ...
        public MainForm()
        {
            ...
            // 注册出图事件
            this.axRenderControl1.RcPictureExportBegin += new Gvitech.CityMaker.Controls._IRenderControlEvents_RcPictureExportBeginEventHandler(axRenderControl1_RcPictureExportBegin);
            this.axRenderControl1.RcPictureExporting += new Gvitech.CityMaker.Controls._IRenderControlEvents_RcPictureExportingEventHandler(axRenderControl1_RcPictureExporting);
            this.axRenderControl1.RcPictureExportEnd += new Gvitech.CityMaker.Controls._IRenderControlEvents_RcPictureExportEndEventHandler(axRenderControl1_RcPictureExportEnd);
            ...
            //// 注册地形
            //string tmpTedPath = Path.Combine(Application.StartupPath.Substring(0, flag), @"Samples\Media\terrain\SingaporePlanarTerrain.ted");
        ...
        private void ExportOrthoImage()
        {
            ...
            if (dlg.ShowDialog() == DialogResult.OK)
            {
                ...
                newEnv.MaxZ = 100;
                bool b = this.axRenderControl1.ExportManager.ExportOrthoImage(dlg.FileName, 1024, center, angle, newEnv, false, 0xff00ffff);
                if (!b)
                {
                    MessageBox.Show("错误码为：" + this.axRenderControl1.GetLastError().ToString());
                }
        ...
        void axRenderControl1_RcPictureExportEnd(object sender, Gvitech.CityMaker.Controls._IRenderControlEvents_RcPictureExportEndEvent e)
        {
        ...
        void axRenderControl1_RcPictureExporting(object sender, Gvitech.CityMaker.Controls._IRenderControlEvents_RcPictureExportingEvent e)
        {
            processdlg.progressBar1.Value = Int16.Parse((100 * e.percentage).ToString());
            processdlg.label1.Text = (100 * e.percentage).ToString();
        }

        void axRenderControl1_RcPictureExportBegin(object sender, Gvitech.CityMaker.Controls._IRenderControlEvents_RcPictureExportBeginEvent e)
        {
        ...
        {
            polyline = e.geometry as IPolyline;
            if (polyline.PointCount == 2)
            {
                ...
                if (dlg.ShowDialog() == DialogResult.OK)
                {
                    bool b = this.axRenderControl1.ExportManager.ExportOrthoImage(dlg.FileName, 1024, center, cameraAngle, newEnv, false, 0xff00ffff);
                    if (!b)
                    {
                        MessageBox.Show("错误码为：" + this.axRenderControl1.GetLastError().ToString());
                    }
